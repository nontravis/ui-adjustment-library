...
public class ColorAdjustment extends BaseAdjustItem<AdjustColor>{
    ...
    public static ColorAdjustment create( @IdRes int id, String title, Map<String, AdjustColor> map, boolean isCommon ){
        if( map == null || map.isEmpty() ) return new ColorAdjustment( id );
        return new ColorAdjustment( id, title, map, isCommon );
    }

    private ColorAdjustment( int id ){
        super( id, BaseAdjustItem.EMPTY, AdjustAdapter.COLOR_ITEM, false );
    }

    private ColorAdjustment( int id, String title, Map<String, AdjustColor> map, boolean isCommon ){
        super( id, title, AdjustAdapter.COLOR_ITEM, isCommon);
        this.mapColor = map;
    }
