...
public class StringAdjustment extends BaseAdjustItem<AdjustString>{
    ...
    public static StringAdjustment create( View view, String title, Map<String, AdjustString> map ){
        return create( view.getId(), title, map );
    }

    public static StringAdjustment create( @IdRes int id, String title, Map<String, AdjustString> map ){
        if( map == null || map.isEmpty() ) return new StringAdjustment( id );

        return new StringAdjustment( id, title, map );
    }

    private StringAdjustment( int id ){
        super( id, BaseAdjustItem.EMPTY, AdjustAdapter.STRING_ITEM );
    }

    private StringAdjustment( int id, String title, Map<String, AdjustString> map ){
        super( id, title, AdjustAdapter.STRING_ITEM );
        this.mapString = map;
    }
